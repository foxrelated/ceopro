...
class Ynultimatevideo_Model_DbTable_Videos extends Engine_Db_Table {
	...
	public function importItem($item, $category_id)
	{
		...
		if ($this->getImportedItem($item)) {
		    ...
		    $aTypes = [1 => 'youtube', 2 => 'vimeo', 3 => 'upload'];
		    $type = $item -> type;
            $code = $item -> code;
		    if (in_array($item -> type, $aTypes)) {
                $type = array_search($item -> type, $aTypes);
            } elseif( $item->type == 'iframely' ) {
                list($type, $code) = Engine_Api::_()->ynultimatevideo()->parseFromVideoCoreToUltimate($type, $code);
            }
