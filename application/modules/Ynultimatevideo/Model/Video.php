...
class Ynultimatevideo_Model_Video extends Core_Model_Item_Abstract
{
    ...
    public function getRichContent($view = false, $params = array())
    {
        ...
        else if ($this -> type == Ynultimatevideo_Plugin_Factory::getVideoURLType())
        {
    ...
    public function getVideo($view = false, $params = array())
    {
        ...
        if ($this->type == 'iframely') {
            list($type, $paramsForCompile['location']) = Engine_Api::_()->ynultimatevideo()->parseFromVideoCoreToUltimate($this->type, $this->code);
        }
    ...
    public function sendEmailToFriends($recipients, $message) {
        ...
        $sentEmails = 0;
        $photo_url = ($this->getPhotoUrl('thumb.profile')) ? $this->getPhotoUrl('thumb.profile') : 'application/modules/Ynultimatevideo/externals/images/nophoto_video_thumb_profile.png';
        foreach( $recipients as $recipient ) {
            $mailType = 'ynultimatevideo_send_video_to_friends';
            $mailParams = array(
