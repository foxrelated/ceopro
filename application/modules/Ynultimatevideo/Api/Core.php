...
class Ynultimatevideo_Api_Core extends Core_Api_Abstract
{
    ...
    /**
     * Function support video core 4.9.x
     *
     * @param $type
     * @param $code
     * @return array
     */
	public function parseFromVideoCoreToUltimate($type, $code)
    {
        if ($type == 'iframely') {
            $regex = '/src=".*\/\/([^\/\/]+)\/.*\/([^"]+)"/';
            preg_match_all($regex, $code, $matches, PREG_SET_ORDER, 0);
            if (!empty($matches[0]) && !empty($matches[0][1]) && !empty($matches[0][2])) {
                $domain = $matches[0][1];
                $code = $matches[0][2];
                if (strpos($domain, 'youtube')) {
                    $type = 1;
                    $code = explode('?', $code)[0];
                } elseif (strpos($domain, 'dailymotion')) {
                    $type = 4;
                } elseif (strpos($domain, 'vimeo')) {
                    $type = 2;
                    $code = explode('?', $code)[0];
                }
            }
