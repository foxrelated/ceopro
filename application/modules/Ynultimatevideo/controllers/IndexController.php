...
class Ynultimatevideo_IndexController extends Core_Controller_Action_Standard
{
	...
	public function createAction()
	{
		...
		try
		{
			...
			if ($values['video_type'] == Ynultimatevideo_Plugin_Factory::getUploadedType())
			{
			...
			else
			{
				...
				$video -> setFromArray($values);
				$video -> type = $values['video_type'];
				if ($values['video_type'] == Ynultimatevideo_Plugin_Factory::getVideoURLType() || $values['video_type'] == 6)
				{
					$video -> status = 1;
				}
				$video -> save();

				if ($values['video_type'] == Ynultimatevideo_Plugin_Factory::getVideoURLType())
				{
					$adapter = Ynultimatevideo_Plugin_Factory::getPlugin((int)$values['video_type']);
		...
		try
		{
			if ($insert_action && $video -> status == 1)
			{
				...
				{
					$action = Engine_Api::_() -> getDbtable('actions', 'activity') -> addActivity($owner, $video, 'ynevent_video_create');
				}
				else
				{
