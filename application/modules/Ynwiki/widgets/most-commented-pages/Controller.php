<?php
class Ynwiki_Widget_MostCommentedPagesController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
    {
       ...
       ->order("$Name.comment_count DESC")->limit($limit);
      
	  $this->view->comments =  $arr_pages = $table->fetchAll($select);
      if(count($arr_pages) <= 0) 
        $this->setNoRender();
	  $css = "global_form_box";
	  $page_listing_id = 'page_listing_most_commented';
