<?php
class Ynwiki_Widget_RecentViewedPagesController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
    {
       ...
       ->join("engine4_ynwiki_views","engine4_ynwiki_views.page_id = $Name.page_id","")
       ->where('engine4_ynwiki_views.user_id = ?',$viewer->getIdentity())
       ->order("engine4_ynwiki_views.creation_date DESC")
       ->limit($limit); 
      ...
      if(count($arr_pages) <= 0)
        $this->setNoRender();
	  $css = "global_form_box";
	  $page_listing_id = 'page_recent_viewed';
