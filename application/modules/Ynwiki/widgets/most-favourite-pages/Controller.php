<?php
class Ynwiki_Widget_MostFavouritePagesController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
    {
       ...
       ->order("$Name.favourite_count DESC")->limit($limit);
      
	  $this->view->comments =  $arr_pages = $table->fetchAll($select);
      if(count($arr_pages) <= 0) 
        $this->setNoRender();
	  $css = "global_form_box";
	  $page_listing_id = 'page_listing_most_favourite';
