	...
	<div class="uiYnfbppHovercardStage">
		...
		<div class="uiYnfbpp_main_info">
			<div class="uiYnfbpp_info_content">
				<?php if($allow && $settings->getSetting('ynfbpp.multilisting.price', 1) && floatval($this->subject->price)): ?>
				...
				<ul class="uiYnfbpp_personal_info clearfix">
					<?php if($allow && $settings->getSetting('ynfbpp.multilisting.enddate', 1) && !empty($this->subject->expiration_date)): ?>
						<li class="uiYnfbpp_fullwidth">
							<span><?php echo $this->translate('Expired') . ': ' ?></span>
							<span class="uiYnfbppexp"><?php echo date('M d Y', strtotime($this->subject->expiration_date)) ?></span>
						</li>
					<?php endif; ?>
					<?php if($allow && $settings->getSetting('ynfbpp.multilisting.description', 1) && !empty($this->subject->description)): ?>
						<li class="uiYnfbpp_personal_decs">
							<span><?php echo strip_tags($this->subject->description) ?></span>
						</li>
					<?php endif; ?>
					<?php if($allow && $settings->getSetting('ynfbpp.multilisting.location', 1) && $this->subject->location): ?>
					...
					<?php endif; ?>
					<?php if($allow && $settings->getSetting('ynfbpp.multilisting.category', 1)): ?>
