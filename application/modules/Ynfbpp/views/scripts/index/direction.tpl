...
$this->headScript()->appendFile("https://maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&libraries=places&key=$googleApi");
?>
<style>
      .controls {
...
<div id="ynfbpp_direction">
  <h4><?php echo $this->translate("Get Direction");?></h4>
<div class="ynfbpp_widget_location_direction">
  ...
  <input id="pac-input" class="controls" placeholder="<?php echo $this->translate("Enter a location"); ?>" type="text" name="starting_location" id="starting_location" />
    <span class="close-btn" id="ynfbpp_location_direction_close_btn">x</span>
</div>
<div class="ynfbpp_widget_location_direction get_current_position">
...
<div class="ynfbpp_widget_location_direction ynfbpp_widget_location_destination">
    ...
    switch ($this->item->getType()) {
        case 'group':
            if (!empty($this->item->location)) {
                $location = json_decode($this->item->location);
                if ($location->{'location'} != "0")
                    $locationText = $location->{'location'};
            }
            break;
        default;
            $locationText = $this->item->location;
    }
    ?>
  <?php echo $locationText ?>
</div>
<div class="ynfbpp_widget_location_direction ynfbpp_widget_location_direction_button">
    <label>&nbsp;</label>
	...
	if(navigator.geolocation) {
	    navigator.geolocation.getCurrentPosition(function(position) {
			...
			var current_posstion = new Request.JSON({
				'format' : 'json',
				'url' : '<?php echo $this->url(array('action'=>'get-my-location'), 'ynfbpp_general') ?>',
				'data' : {
...
{
	$('ynfbpp_location_direction_close_btn').addEvent('click', function(e) {
		$('pac-input').set('value','');
		fromPoint = null;
    });
});
