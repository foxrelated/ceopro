...
<div class='generic_layout_container layout_middle ynbusinesspages_photo_list_container'>
	...
	<div class="ynbusinesspages-profile-module-header">
        ...
        <div class="ynbusinesspages-profile-header-right">
			...
			if($session -> mobile)
			  	echo $this->htmlLink(array('route' => 'ynbusinesspages_profile', 'id' => $this->business->getIdentity()), '<i class="ynicon yn-arrow-left"></i>'.$this->translate('Back to Business'), array(
			    'class' => 'buttonlink'
			  ));
			else {
				echo $this->htmlLink(array('route' => 'ynbusinesspages_profile', 'id' => $this->business->getIdentity(), 'slug' => $this->business-> getSlug(), 'tab' => $this -> tab), '<i class="ynicon yn-arrow-left"></i>'.$this->translate('Back to Business'), array(
			    'class' => 'buttonlink'
			  ));
			} ?>
	    ...
	    foreach( $this->paginator as $photo ): ?>
	      <li id='thumbs_nocaptions_<?php echo $photo->getIdentity()?>'>
	        ...
	        <p id="nocaptions_photo_<?php echo $photo->getIdentity()?>">
	        	...
	        	<a id="nocaptions_photo_remofile_<?php echo $photo->getIdentity()?>" class="buttonlink" onclick="return removeFile(<?php echo $photo->getIdentity()?>)" href="javascript:void();" ><i class="ynicon yn-del"></i> <?php echo $this->translate("Remove")?></a>
	       		<?php endif;?>
	        </p>
   	...
   	function removeFile(photo_id)
   	{
   		...
   		if(action)
   		{
   			request = new Request.JSON({
				'format' : 'json',
	            'url' :  en4.core.baseUrl + 'business-page/photo/delete-photo',
	            'data': {
	            	'photo_id' : photo_id,
