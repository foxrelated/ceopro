			...
			<div class="ynbusinesspages-company-item">
				...
				<div class="ynbusinesspages-company-item-owner-action">
					...
					?>" href="<?php echo $this -> url($aDeleteButton['params'], $aDeleteButton['route'], array()); ?>" > 
						<i class="ynicon yn-trash-alt"></i> <?php echo $this -> translate($aDeleteButton['label']) ?>
					</a>
					<?php endif;?>
				...
				<div class="ynbusinesspages-company-item-content">
                    ...
                    <?php
                    $expire_time = '';
                    if ($business->never_expire){
                        $expire_time = $this->translate('never expire');
                    } else {
                        $expire_time = (!is_null($expirationDateObj)) ? date('M d Y', $expirationDateObj -> getTimestamp())  : '';
                    }
                    $approve_time = (!is_null($approvedDateObj)) ?  date('M d Y', $approvedDateObj -> getTimestamp()) : '';
                    ?>
                    <?php if($expire_time): ?>
					<div class="ynbusinesspages-company-item-date-exp">
						<span class="ynbusinesspages-grey">
                            <?php echo $this -> translate('Expire') ;?>:
                        </span>
                        <?php echo $expire_time; ?>
                        <?php if($approve_time): ?>
                            <?php echo '('.$this -> translate('Approved At: ') . $approve_time . ')'; ?>
                        <?php endif; ?>
					</div>
                    <?php endif; ?>
                    ...
                    <?php if($package -> getIdentity()): ?>
					    <div class="ynbusinesspages-company-item-package-exp"><span class="ynbusinesspages-grey"><?php echo $this -> translate('Package') ;?>:</span> <span><?php echo $this -> translate($package -> getTitle());?></span></div>
                    <?php endif; ?>
                    <?php if(!is_null($lastPaymentDateObj)): ?>
					    <div class="ynbusinesspages-company-item-last-payment"><span class="ynbusinesspages-grey"><?php echo $this -> translate('Last Payment') ;?>:</span> <b><?php echo date('M d Y', $lastPaymentDateObj -> getTimestamp()); ?></b></div>
                    <?php endif; ?>
