            ...
            <div class="ynultimatevideo_info_bot">
                <span class="ynultimatevideo_duration">
                    <?php if ($video->duration): ?>
                        <?php echo '<i class="ynicon yn-clock-o"></i>'.$this->partial('_video_duration.tpl', 'ynultimatevideo', array('video' => $video)) ?>
                    <?php else: ?>
                        <i class="ynicon yn-clock-o"></i>00:00
                    <?php endif ?>
                </span>
                        ...
                        <div class="video-action-add-playlist">
                            <div class="video-action-watch-later" onclick="ynultimatevideoAddToWatchLater(this, '<?php echo $video -> getIdentity() ?>');">
                                <?php if ($addedWatchlater): ?>
                                    <i class="ynicon yn-stop"></i>
                                    <?php echo $this->translate('Unwatched') ?>
                                <?php else: ?>
                                    <i class="ynicon yn-play-circle"></i>
            ...
            <div class="ynultimatevideo_author clearfix">
                ...
                <span class="ynultimatevideo_views">
                        <span class="ynultimatevideo_count">
                            <?php echo '<i class="ynicon yn-eye"></i>'.$this->translate(array('%1$s', '%1$s', $video->view_count), $this->locale()->toNumber($video->view_count)) ?>
                        </span>
                        <span class="ynultimatevideo_count">
                            <?php
                                $likeCount = $video->likes()->getLikeCount();
                                echo '<i class="ynicon yn-heart"></i>'.$this->translate(array('%1$s', '%1$s', $likeCount), $this->locale()->toNumber($likeCount));
                            ?>
                        </span>
                         <span class="ynultimatevideo_count">
                            <?php
                                $commentCount = $video->comments()->getCommentCount();
                                echo '<i class="ynicon yn-comments"></i>'.$this->translate(array('%1$s', '%1$s', $commentCount), $this->locale()->toNumber($commentCount));
                            ?>
                        </span>
                </span>
            </div>
