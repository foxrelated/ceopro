...
class Ynbusinesspages_Model_DbTable_Mappings extends Engine_Db_Table
{
    ...
    public function getGroupbuysSelect($params = array())
    {
        ...
        $tableLocation = Engine_Api::_()->getItemTable('groupbuy_location');
        $locationName = $tableLocation->info('name');

        if (Engine_Api::_()->hasModuleBootstrap('ynlocationbased') && Engine_Api::_()->hasModuleBootstrap('groupbuy')) {
            $select = Engine_Api::_()->ynlocationbased()->getLocationBasedSelect('groupbuy', 'deals');
        } else {
            $select = $table->select()->from($rName);
        }
        $select -> setIntegrityCheck(false);

        $select -> joinLeft($locationName, "$rName.location_id = $locationName.location_id", "$locationName.title as location_title");
        if (!isset($params['ItemTable'])) {
            $params['ItemTable'] = 'groupbuy_deal';
        }
        $ids = $this->getItemIdsMapping($params['ItemTable'], $params);
        if (!empty($ids)) {
            $select->where("$rName.deal_id IN (?)", $ids);
        } else {
            $select->where("$rName.deal_id IN (0)", $ids);
        }
        //Search
        if (!empty($params['search'])) {
        ...
        if (isset($params['manage']) && $params['manage']) {
            $select->where(new Zend_Db_Expr("IF($rName.user_id = $user_id, 1, $rName.published = 20 AND $rName.status IN (20,30) AND $rName.start_time <= '$cur_time' AND $rName.end_time >= '$cur_time' AND $rName.current_sold < $rName.max_sold)"));
        } else {
            $select->where("$rName.published = 20 AND $rName.status IN (20,30) AND $rName.start_time <= '$cur_time' AND $rName.end_time >= '$cur_time' AND $rName.current_sold < $rName.max_sold");
        }
    ...
    public function getContestsSelect($params = array())
    {
        ...
        // Browse by
        if (isset($params['browseby']) && $params['browseby'] != "") {
            if ($params['browseby'] == "feature")
                $select->where("featured_id = 1");
            if ($params['browseby'] == "premium")
                $select->where("premium_id =  1");
            if ($params['browseby'] == "endingsoon")
    ...
    public function getListingsSelect($params = array())
    {
        ...
        if (Engine_Api::_()->hasModuleBootstrap('ynlocationbased')) {
            ...
            $select = $table->select()->from($Name, array("$Name.*", "COUNT($postTblName.post_id) as discuss_count"));
        }

        $select->setIntegrityCheck(false);
        $select->joinLeft("$postTblName", "$postTblName.listing_id = $Name.listing_id", "");
        $select->group("$Name.listing_id");
        if (!isset($params['ItemTable'])) {
            $params['ItemTable'] = 'ynlistings_listing';
        }
        $ids = $this->getItemIdsMapping($params['ItemTable'], $params);
        if (!empty($ids)) {
            $select->where("$Name.listing_id IN (?)", $ids);
        } else {
            $select->where("$Name.listing_id IN (0)", $ids);
        }
        //Search
        if (!empty($params['search'])) {
            $select->where("$Name.title LIKE ?", '%' . $params['search'] . '%');
        }
        
        $viewer = Engine_Api::_()->user()->getViewer();
        $user_id = $viewer->getIdentity();
        if (isset($params['manage']) && $params['manage']) {
            $select->where(new Zend_Db_Expr("IF($Name.user_id = $user_id, 1, $Name.search = 1 AND $Name.status = 'open' AND $Name.approved_status = 'approved')"));
        } else {
            $select
                ->where("$Name.search = ?", 1)
                ->where("$Name.status = ?", 'open')
                ->where("$Name.approved_status = ?", 'approved');
        }
        // Order
        switch ($params['order']) {
            case 'recent':
                $select->order("$Name.listing_id DESC");
                break;
            case 'view':
                $select->order("$Name.view_count DESC");
                break;
            case 'like':
                $select->order("$Name.like_count DESC");
                break;
            case 'discussion':
                $select->order("discuss_count DESC");
                break;
            case 'title':
                $select->order("$Name.title ASC");
                break;
            default:
                $select->order("$Name.listing_id DESC");
                break;
    ...
    public function getJobsSelect($params = array())
    {
        ...
        if (Engine_Api::_()->hasModuleBootstrap('ynlocationbased')) {
            $select = Engine_Api::_()->ynlocationbased()->getLocationBasedSelect('ynjobposting', 'jobs');
        } else {
