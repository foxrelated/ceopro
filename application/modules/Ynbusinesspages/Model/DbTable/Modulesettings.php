...
class Ynbusinesspages_Model_DbTable_Modulesettings extends Engine_Db_Table 
{
	...
	public function getEnabledModuleSettings()
	{
		...
		foreach ($this -> fetchAll($this -> select()) as $row)
		{
            ...
            else if (in_array($row->key, array('video_create', 'video_delete'))) {
                if (Engine_Api::_()->hasModuleBootstrap('video') || Engine_Api::_()->hasModuleBootstrap('ynultimatevideo') || Engine_Api::_()->hasModuleBootstrap('ynvideochannel'))
                    $settings[] = $row;
            }
