...
class Ynbusinesspages_Plugin_Core
{
		...
		if (is_object($request))
		{
			...
			if ($type == 'ynbusinesspages_business')
			{
				if ($business_id)
				{
					...
					Zend_Registry::get('Zend_Log') -> log(print_r('Typeaaa: '.$case, true), Zend_Log::DEBUG);
					switch ($case) 
					{								
					case 'folder':
					...
					case 'ynultimatevideo_video':
						...
						$_SESSION[$key] = $value;
						break;
					case 'ynvideochannel_video':
						$key = 'ynbusinesspages_predispatch_url:' . $request->getParam('module') . '.index.manage-videos';
						$value = $view->url(array(
	...
	public function onItemCreateAfter($event)
	{
		...
		if($payload -> getType() == 'activity_action')
		{
			...
			if($object)
			{
					...
					&& (!in_array($payload -> type, array('post', 'post_self')) || (in_array($payload -> type, array('post', 'post_self')) && $payload -> object_type == 'ynbusinesspages_business')))
				{
					...
					/*
					 * Fix bug Fatal Error on social engine 4.9:
					 * Issues occur when we save an item. And when saving, we save it again on hook onItemCreateAfter()
					 * Please note that social engine 4.9 change order function _postInsert() and _refresh() when
					 * function Core_Model_Item_Abstract->save() process.
					 * Step 1: After created a blog. We'll call to addActivity(). And activity model'll call Core_Model_Item_Abstract->save().
					 * On Core_Model_Item_Abstract->save(), function _postInsert() call, it trigger event onItemCreateAfter().
					 * Step 2: Then it go here and our module call Core_Model_Item_Abstract->save() again (line 214 $payload -> save();)
					 * Step 3: It'll back to Step 1 and a loop generated again and again.
					 * Solution: We have to call $payload->refresh(); before we save something on hook onItemCreateAfter().
					 * If not. Fatal Error will happen.
					 */
                    if ($payload instanceof Zend_Db_Table_Row_Abstract) {
                        $payload->refresh();
                        $payload -> save();
                    }
				}
		...
		$view = Zend_Registry::get('Zend_View');
        Zend_Registry::get('Zend_Log') -> log(print_r('Typeaaa6: '.$payload -> getType(), true), Zend_Log::DEBUG);
		if($business_id)
		{
            ...
            Zend_Registry::get('Zend_Log') -> log(print_r('Typeaaa7: '.$payload -> getType(), true), Zend_Log::DEBUG);
			switch ($payload -> getType()) 
			{
				...
				case 'ynvideochannel_video':
                    ...
                    if ($type == 'contest') $type = 'yncontest_contest';
					$row = $table -> createRow();
                    Zend_Registry::get('Zend_Log') -> log(print_r('Typeaaa9: '.$payload -> getType(), true), Zend_Log::DEBUG);
					...
					$row -> save();
					Zend_Registry::get('Zend_Log') -> log(print_r('Typeaaa10: '.$payload -> getType(), true), Zend_Log::DEBUG);
					...
					else if($payload -> getType() == 'ynvideochannel_video') {
						Zend_Registry::get('Zend_Log') -> log(print_r($payload -> getType(), true), Zend_Log::DEBUG);
						// send notification to followers
						$business -> sendNotificationToFollowers($view -> translate('new videochannel video'));
						$key = 'ynbusinesspages_predispatch_url:' . $request -> getParam('module') . '.video.detail';
						$value = $view -> url(array(
							'controller' => 'video-channel',
							'action' => 'list',
							'business_id' => $business_id,
			...
			if ($type == 'ynbusinesspages_business')
			{
				...
				if ($business_id)
				{
					...
					switch ($type) 
					{
						case 'video':
							$row = $table -> createRow();
							$video_owner_id = $payload -> owner_id;
							if (!$video_owner_id)
								$video_owner_id = Engine_Api::_()->user()->getViewer()->getIdentity();
						    $row -> setFromArray(array(
						       'business_id' => $business_id,
						       'item_id' => $payload -> getIdentity(),
						       'type' => 'video',
                        ...
                        case 'ynvideochannel_video':
                            $action = $activityApi->addActivity($viewer, $business, 'ynbusinesspages_ynvideochannel_video_create');
                            if($action) {
                                $activityApi->attachActivity($action, $payload);
                            }
                            $key = 'ynbusinesspages_predispatch_url:' . $request -> getParam('module') . '.video.detail';
