...
if (empty($request) || (!($module == "default" && $controller == "sdk" && $action == "build") && !($module == "default" && $controller == "manage" && $action == "prepare"))) {
...
return array (
  ...
  'routes' => array(
	...
	'ynbusinesspages_specific' => array(
			...
			'reqs' => array(				
					'action' => '(open-close|delete|transfer|tooltip|edit|add-to-compare|profile-follow|profile-favourite|un-follow|un-favourite|promote|checkin|remove-item|get-people-checkin|load-more-photos)',
					'business_id' => '\d+',
			)
	),
