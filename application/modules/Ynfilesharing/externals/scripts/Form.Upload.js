...
Form.Upload = new Class({
	...
	modernUpload: function(input){
		...
		var progress_background = new Element('div.ynfs_progress_background').inject(list, 'after');
		var progress = new Element('div.ynfs_progress').setStyle('display', 'none').inject(progress_background);
		var progress_percent = new Element('p#progress_percent').inject(progress_background);
		...
		var uploadReq = new Request.File({
			url: uri.toString(),
			onRequest: progress.setStyles.pass({display: 'block', width: 0, margin: 0}, progress),
			onProgress: function(event){
				var loaded = event.loaded, total = event.total;
				progress.setStyle('width', parseInt(loaded / total * 100, 10).limit(0, 100) + '%');
				progress_percent.innerHTML = parseInt(loaded / total * 100, 10).limit(0, 100) + '%';
			},
			onComplete: function(){
				progress.setStyle('width', '100%');
				progress.setStyle('display', 'none');
				progress_percent.setStyle('display', 'none');
