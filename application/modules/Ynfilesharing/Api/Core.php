...
class Ynfilesharing_Api_Core extends Core_Api_Abstract
{
	...
	public function getSubFolders($folder = NULL, $parent = NULL)
	{
		...
		$folderSelect = $folderTbl -> select()->order(new Zend_Db_Expr('title ASC'));
	...
	public function getFolders($folders, $action = 'view', $viewer = NULL, $from = 0, $limit = NULL)
	{
		...
		$subFolders = array();
        if ($from > 0) {
            $fromFolder = Engine_Api::_()->getItem('ynfilesharing_folder', $from);
        }

		foreach ($folders as $folder)
		{
            //if ($folder -> getIdentity() < $from)
            if (!empty($fromFolder) && strcmp($fromFolder->getTitle(), $folder->getTitle()) > 0)
			{
				continue;
			}
			else
	...
	{
		$fileTbl = new Ynfilesharing_Model_DbTable_Files();
		$fileSelect = $fileTbl -> select() -> order(new Zend_Db_Expr('name ASC'));
    ...
    {
        return $this->selectFoldersByOptions($params);
    }

	public function selectFoldersByOptions($params = array())
	{
		// Get filesharing table
	...
	public function checkFileExists($file_name, $folder_id)
	{
		$fileTable = Engine_Api::_() -> getDbTable('files', 'ynfilesharing');
		$select = $fileTable -> select() -> where('name = ?', $file_name) -> where("folder_id = ?", $folder_id) -> limit(1);
		return $fileTable -> fetchRow($select)?true:false;
	}
