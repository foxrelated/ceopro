...
class Ynfilesharing_FolderController extends Core_Controller_Action_Standard
{
     ...
     * @param string $action
     */
    private function _setAuth($folder, $auth = NULL, $parentType, $parentID, $api, $action)
    {
        if ($parentType == 'group') {
        ...
        if ($auth == NULL)
        {
            ...
            $options = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('folder', $id, 'auth_'.$action);
            if(count($options) == 1)
            {
                $auth = $options[0];
            }
        ...
        if (null !== ($owner_item = Engine_Api::_() -> getItem($parentType, $parentID))
            && null !== ($owner = $owner_item -> getOwner())
            && !$folder->authorization()->isAllowed($owner, $action)) {
            $api->setAllowed($folder, $owner, $action, 1);
        }
    }
        ...
        if (isset ($params ['type'])) {
            switch ($params ['type']) {
                case 'file' :
                    ...
                    break;
                case 'folder' :
                    $folders = $filesharingApi->selectFoldersByOptions($params);
                    break;
                case 'all' :
                    $files = $filesharingApi->selectFilesByOptions($params);
                    $folders = $filesharingApi->selectFoldersByOptions($params);
    ...
    public function createAction()
    {
        ...
        if ($result != $title) {
            $form->getElement('title')->addError(Zend_Registry::get('Zend_View')
                ->translate('A folder name can\'t contain any of the following character: %s', implode(' ', $bad)));
            return false;
        }
        ...
        if ($existingFolder != NULL) {
            ...
            try {
                ...
                $this->_setAuth($folder, $values['auth_view'], $this->_parentType,$this->_parentId, $authApi, 'view');
                $this->_setAuth($folder, $values['auth_create'], $this->_parentType,$this->_parentId, $authApi, 'create');
                $this->_setAuth($folder, $values['auth_edit'], $this->_parentType,$this->_parentId, $authApi, 'edit');
                $this->_setAuth($folder, $values['auth_delete'], $this->_parentType,$this->_parentId, $authApi, 'delete');
                $this->_setAuth($folder, $values['auth_comment'], $this->_parentType,$this->_parentId, $authApi, 'comment');
                $this->_setAuth($folder, $values['auth_edit_perm'], $this->_parentType,$this->_parentId, $authApi, 'edit_perm');
        ...
        if ($parent != NULL) {
            ...
            if ($parent->user_id != $this->_viewer->getIdentity()) {
                $folderOwner = Engine_Api::_()->getItem('user', $parent->user_id);
                $notifyApi = Engine_Api::_() -> getDbtable('notifications', 'activity');
                $notifyApi->addNotification($folderOwner, $this->_viewer, $parent, 'ynfilesharing_folder_create', array('label'=>$parent->getTitle()));
            }
    ...
    public function editPermAction()
    {
        ...
        try {
            $folder->setFromArray($values);
            $folder->save();
            $this->_setAuth($folder, $values['auth_view'], $folder->parent_type,$this->_parentId, $authApi, 'view');
            $this->_setAuth($folder, $values['auth_create'], $folder->parent_type,$this->_parentId, $authApi, 'create');
            $this->_setAuth($folder, $values['auth_edit'], $folder->parent_type,$this->_parentId, $authApi, 'edit');
            $this->_setAuth($folder, $values['auth_delete'], $folder->parent_type,$this->_parentId, $authApi, 'delete');
            $this->_setAuth($folder, $values['auth_comment'], $folder->parent_type,$this->_parentId, $authApi, 'comment');
    ...
    public function uploadAction()
    {
        ...
        foreach ($files as $file => $info) {
            ...
            $counter = 0;
            if(Engine_Api::_() -> ynfilesharing() -> checkFileExists($filename, $folderId))
                $counter = 1;
            do
            {
            ...
            while(Engine_Api::_() -> ynfilesharing() -> checkFileExists($filename, $folderId));
            ...
            if (strpos($fileExts, $fileExtend) !== false || $fileExts == "*") {
                if ($info['size'] < $maxFileSize || $maxFileSize == 0)
                {
                    ...
                    if($counter == 1)
                    {
                        $new_filename = str_replace(' (0)', '', $filename);
                        rename($destination . $filename, $destination . $new_filename);
                    ...
                    try {
                        ...
                        if (in_array($fileExtend, $arr_ext) && $api_viewer && $scribd_api_key && $scribd_secret) {
        ...
        $this->view->error = 0;
        if ($uploadedFiles > 0 && $folder->user_id != $this->_viewer->getIdentity()) {
            $folderOwner = Engine_Api::_()->getItem('user', $folder->user_id);
            $notifyApi = Engine_Api::_() -> getDbtable('notifications', 'activity');
            $notifyApi->addNotification($folderOwner, $this->_viewer, $folder, 'ynfilesharing_file_create', array('label'=>$folder->getTitle(), 'count'=>$uploadedFiles));
        }
