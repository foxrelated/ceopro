...
class Ynfilesharing_FileController extends Core_Controller_Action_Standard
{
	...
	public function editAction()
	{
		...
		&& $this -> view -> form -> isValid($this -> getRequest() -> getPost()))
		{
			...
			if ($file)
			{
				if(Engine_Api::_() -> ynfilesharing() -> checkFileExists($title . '.' . $file -> ext, $file -> folder_id))
				{
					$form->getElement('name')->addError(Zend_Registry::get('Zend_Translate')->_('A file name is exists in this folder!'));
					return false;
				}
	...
	function deleteAction()
	{
		...
		if ($this -> getRequest() -> isPost())
		{
			...
			try
			{
				...
				if ($parentFolder && $parentFolder -> isAllowed($viewer, 'view'))
				{
					$file -> delete();
