...
var YnAdsAutocompleter = new Class({
    ...
    initialize: function (element, options) {
        this.isSearchMini = false;
        if(element == 'search_mini_field')
            this.isSearchMini = true;
    ...
    prefetch: function () {
        ...
        if(!this.isSearchMini && ($$('.ynhashtag-autosearch-global').length || $$('.ynhashtag-autosearch-form').length)) {
            var temp_query = query;
            temp_query = temp_query.replace(/\s/g, '');
    ...
    update: function (tokens) {
        // check conflict with hashtag search
        if(!this.isSearchMini && ($$('.ynhashtag-autosearch-global').length || $$('.ynhashtag-autosearch-form').length)) {
            var temp_query = this.element.value;
            temp_query = temp_query.replace(/\s/g, '');
            var pos = temp_query.lastIndexOf('#');
            if (pos == 0) {
                this.hideChoices(true);
