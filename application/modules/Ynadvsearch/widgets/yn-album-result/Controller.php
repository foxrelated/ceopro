<?php
class Ynadvsearch_Widget_YnAlbumResultController extends Engine_Content_Widget_Abstract
{
	public function indexAction()
	{
		...
		$check_advalbum = Engine_Api::_() -> ynadvsearch() -> checkYouNetPlugin('advalbum');
		if(!$check_advalbum)
		{
			return $this -> setNoRender();
		}
...
//		));
            $this->view->paginator= $paginator;
			$this->view->album_listing_id= $album_listing_id;
			$this->view->no_albums_message= $no_albums_message;
			$this->view->short_title= 1;
			$this->view->no_bottom_space= 1;
			$this->view->css= (!empty($widget_params['title']))?"":'no_title';
	    ...
	    unset($p['action']);
	    unset($p['rewrite']);
	    $this->view->formValues = array_filter($p);

		 //count result
		$ori_select = $table -> select() -> where("search = 1") -> order($order . ' DESC');
		if ($album_privacy) {
            $viewer = Engine_Api::_() -> user() -> getViewer();
            $albums = $table -> fetchAll($ori_select);
            $allowedAlbums = array();
            foreach ($albums as $a)
            {
                if ($a -> authorization() -> isAllowed($viewer, 'view'))
            ...
            $this->view->total_content = Zend_Paginator::factory($allowedAlbums)->getTotalItemCount();
        }
        else
        {
            $this->view->total_content = Zend_Paginator::factory($ori_select)->getTotalItemCount();
        }
	    $this->view->total_content = Engine_Api::_()->ynadvsearch()->countItemByItemType('advalbum_album');
