<?php
    ...
    $this->addHelperPath(APPLICATION_PATH . '/application/modules/Advmenusystem/View/Helper', 'Advmenusystem_View_Helper');
?>
			...
			<?php foreach ($this->menuLevel1 as $k => $menu1):?>
				...
				<li class="ynmenu-<?php echo $menuType;?> ynmenu-level1-idmenu-<?php echo $menu1->id;?>" data-level="1">
					<span class="toggle"></span>
                        ...
                        >
                        <!-- icon -->
                        <span class="ynmenu-icon"><?php echo $this->parseIcon($icon, 'fa-list-alt');?></span>

                        <!-- hover-active icon -->
                        <span class="ynmenu-icon-hover"><?php echo $this->parseIcon($hover_active_icon, 'fa-list-alt');?></span>
						...
						<?php foreach ($this->menuLevel1 as $k => $menu1):?>
							...
							<?php if ($k >= $this->number_tabs): ?>
								<li data-level=2 data-level="2">
                                    ...
                                    >
										<!-- icon -->
										<span class="ynmenu-icon"><?php echo $this->parseIcon($icon, 'fa-cog');?></span>

										<!-- hover-active icon -->
										<span class="ynmenu-icon-hover"><?php echo $this->parseIcon($hover_active_icon, 'fa-cog');?></span>
	...
	window.addEvent('scroll',function(e) {
	    ...
	    if ( window.pageYOffset > check.getPosition().y ) {
	    	if ( !menu.hasClass('menu-fixed') ) {
	    		...
	    		// improve
	    		if ( check.length && ynmenu_container.x > 750 ) {
