                ...
                if ($action && $attachment) {
                    if(is_array($attachment))
                    {
                        foreach($attachment as $attachment_item) {
                            Engine_Api::_() -> getDbtable('actions', 'ynfeed') -> attachActivity($action, $attachment_item);
                        }
    ...
    public function editPostAction()
    {
        ...
        if (!$this -> getRequest() -> isPost()) 
        {
            ...
            if ($viewer -> getIdentity()) 
            {
                ...
                // Checkin JS
                $headScript -> appendFile("//maps.googleapis.com/maps/api/js?key=". Engine_APi::_()->getApi('settings', 'core')->getSetting('yncore.google.api.key', 'AIzaSyB3LowZcG12R1nclRd9NrwRgIxZNxLMjgc')."&v=3.exp&libraries=places");
