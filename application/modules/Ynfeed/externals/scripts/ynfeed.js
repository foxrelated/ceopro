...
function ynfeed(input, active_tags, is_edit, input_hidden)
{
	...
	function takeMap()
	{
		...
		for(var i = 0; i < len; ++i)
		{
			...
			res += kk + ks[i];
			if(vs[i]){
				last_hash = kk.lastIndexOf('#hashtags@');
				if(kk.substr(last_hash) == "#hashtags@" && ks[i].substr(0,14) == '#tags@taguser@')
				{
					hv += kk.substr(0, last_hash) + '#'+'<a href="' + (ks[i].replace(/;.+$/, '') ) + '">' + vs[i] + '</a>';
					ambiguous = true;
				}else if(kk.substr(0,10) == "#hashtags@" 
					&& kk.slice(-1) != ' ' 
					&& ks[i].substr(0,14) == '#tags@taguser@' 
					&& kk.substr(last_hash + 10).indexOf('@') < 0){
					hv += kk.substr(0, last_hash) + '#' + kk.substr(last_hash + 10) + '<a href="' + (ks[i].replace(/;.+$/, '') ) + '">' + vs[i] + '</a>';
					ambiguous = true;
				}else{					
		...
		if(i > 0) {
			...
			if(ambiguous && ces.slice(-1) == '#')
				ces = ces.substr(0, ces.length-1);
			res += ces
			hv += ces;
		}
		hi2.value = res;
	...
	function takeHashMap()
	{
		...
		if(ks != null)
		{
			...
			for(var i = 0; i < len; ++i)
			{
				...
				// make sure there is no special character in hashtag
				var ks_clean = ks[i].replace(/\#/, '');
				
				if(!alphabet_regex.test(ks_clean)) {
				    res += kk + ks[i];
					hv += kk + ks[i];
				}else{
