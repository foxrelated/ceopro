...
class SdkController extends Zend_Controller_Action
{
  ...
  public function createAction()
  {
    ...
    if( file_exists($skeletonPath) ) {
      $it = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($skeletonPath), RecursiveIteratorIterator::CHILD_FIRST);
      foreach( $it as $file ) {
    ...
    if( $manifestData['type'] == 'language' ) {
      $targetPath = $archiveDirectory . '/' . 'application/languages/en/' . $values['name'] . '.csv';
      $targetData = '';
      if( !is_dir(dirname($targetPath)) ) {
        if( !mkdir(dirname($targetPath), 0777, true) ) {
          throw new Engine_Exception(sprintf('Unable to create folder: %s', dirname($targetPath)));
