...
var Tagger = new Class({
  ...
  initialize : function(el, options) {
    ...
    if (this.element.getParents('.ynadvalbum_view_photo_popup').length) {
  ...
  onMove : function(coords) {
    ...
    if (this.options.isPopup) {
      if ((coords.y) > (this.element.getSize().y - 260)) {
        newTop = pos.y + coords.y - 200;
      } else {
        newTop = pos.y + coords.y + 20;
      }
      newLeft = pos.x + coords.x + coords.w + 20;
    } else {
      if ((coords.x + coords.w) > (this.element.getSize().x - 120)) {
        newLeft = pos.x + coords.x - 170;
      } else {
        newLeft = pos.x + coords.x + coords.w + 20;
      }
      if ((coords.y) > (this.element.getSize().y - 120)) {
        newTop = pos.y + coords.y - 200;
      } else {
        newTop = pos.y + coords.y + 20;
      }
  ...
  addTag : function(params) {
    ...
    if (this.options.popupTagList) {
      this.getTagText().setStyle('display', '');
      this.refreshTagList();
    }
  },

  refreshTagList: function() {
    var tagList = this.getTagList().getChildren('.tag_info');
    var tagText = this.getTagText();
    tagText.getChildren().destroy();

    if (tagList.length) {
      var taggedText = new Element('span', {
        'class' : 'ynadvalbum_tagged',
        'html' : en4.core.language.translate('Tagged:'),
      }).inject(tagText);
      var firstTag = tagList[0];
      var cloned = firstTag.clone(true, true);
      var tagId = firstTag.get('id');
      var id = tagId.substring(9);

      cloned.set('id', tagId + '_cloned');
      cloned.addEvent('mouseover', function(){
        this.showTag(id);
      }.bind(this));
      cloned.addEvent('mouseout', function(){
        this.hideTag(id);
      }.bind(this));
      var destroyer = cloned.getChildren('.tag_destroyer');
      destroyer.addEvent('click', function() {
        this.removeTag(id);
      }.bind(this));
      cloned.inject(tagText);

      // add others... text
      if (tagList.length > 1) {
        var remainTagsCount = tagList.length - 1;
        var otherText = (remainTagsCount > 1) ? ' others' : ' other';
        var html = ' and <a href="javascript:void(0)" onclick="ynadvalbumOpenTagBox()">' + remainTagsCount + ' ' + otherText + '</a>';
        var remainTagsText = new Element('span', {
          'class' : 'ynadvalbum_tagged_remain',
          'html' : html
        }).inject(tagText);
      }
    }
  ...
  getTagText : function() {
    if( !this.tagText ) {
      this.tagText = $(this.options.tagTextElement);
    }
    return this.tagText;
  },
  ...
  removeTag : function(id) {
    ...
    // custom
    if ($('tag_info_' + id + '_cloned'))
      $('tag_info_' + id + '_cloned').destroy();
    this.count--;

    if (this.options.popupTagList) {
      if (this.count < 2) {
        ynadvalbumCloseTagBox();
      }
