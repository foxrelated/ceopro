...
class Advmenusystem_AdminMenusController extends Core_Controller_Action_Admin
{
	...
	public function createAction()
	{
        ...
        // process icon
        foreach (array('icon', 'hover_active_icon') as $icon) {
            if (($values[$icon] == 'icon_class') && !empty($values[$icon . '_class']))
            {
                $values[$icon] = $values[$icon . '_class'];
            }
        }
	...
	public function editAction()
	{
		...
		if (isset($menuItemData['params']) && is_array($menuItemData['params'])) {
            ...
            // process icon
            foreach (array('icon', 'hover_active_icon') as $icon) {
                if (!empty($menuItemData[$icon])) {
                    if (substr($menuItemData[$icon], 0, 3) == 'fa-') {
                        $menuItemData[$icon . '_class'] = $menuItemData[$icon];
                        $menuItemData[$icon] = 'icon_class';
                    }
                }
            }
        }
