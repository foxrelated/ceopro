	...
	<?php foreach( $this->navigation as $item ):
		if($item -> name == 'core_mini_auth' || $item -> name == 'core_mini_signup'):?>
			...
			<li class="">
				<a 
					<?php 
						if($item -> name == 'core_mini_auth' && !empty($this->advmenusystemEnableLoginLightbox) && (empty($this->isUserLoginPage) && empty($this->isUserSignupPage)) ){
							echo 'class="smoothbox"';
							$href = $this->url(array('module' => 'advmenusystem', 'controller'=>'login', 'action'=>'index', 'case'=>1), 'advmenusystem_minimenu');
						}
						elseif($item -> name == 'core_mini_signup' && !empty($this->advmenusystemEnableSignupLightbox) && (empty($this->isUserSignupPage) && empty($this->isUserLoginPage))){
							$href = $this->url(array('module' => 'advmenusystem', 'controller'=>'signup', 'action'=>'index', 'case'=>1), 'advmenusystem_minimenu');	
							echo 'class="smoothbox"';
						}
						else{
							$href = $item->getHref();
